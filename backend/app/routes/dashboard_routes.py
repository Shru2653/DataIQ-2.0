"""
dashboard_routes.py
POST /api/dashboard/auto

Registered in main.py as:
    app.include_router(dashboard_router)          ← no prefix (matches all other routers)

The /api prefix lives in the decorator below, exactly like your other routes.
"""
...
class KPI(BaseModel):
    ...
    color: str = "blue"   # blue | green | amber | red | purple
        ...
        if alt.exists():
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Please upload it first on the Home page."
                ),
            )
...
def _build_kpis(df: pd.DataFrame, col_meta: Dict[str, ColumnMeta], outlier_cnts: Dict[str, int]) -> List[KPI]:
    kpis: List[KPI] = []
    ...
    kpis.append(KPI(
        ...
        value=f"{missing_pct}%",
        sub=f"{total_missing:,} cells",
        color="red" if missing_pct > 10 else "green",
    ))

    dup = int(df.duplicated().sum())
    kpis.append(KPI(label="Duplicate Rows", value=f"{dup:,}",
                    color="amber" if dup > 0 else "green"))
    ...
    for col in [c for c, m in col_meta.items() if m.is_numeric][:4]:
        ...
        kpis.append(KPI(
            label=f"Avg {col}",
            value=_fmt(s.mean()),
            sub=f"max {_fmt(s.max())} · min {_fmt(s.min())}",
            color="blue",
        ))
    ...
    for col in [c for c, m in col_meta.items() if m.is_categorical and not m.is_numeric][:2]:
        ...
        kpis.append(KPI(
            label=f"Top {col}",
            value=str(vc.index[0])[:20],
            sub=f"{round(vc.iloc[0]/len(df)*100,1)}% of rows",
            color="green",
        ))
    ...
    if outlier_cnts:
        kpis.append(KPI(
            label="Outlier Rows",
            value=f"{sum(outlier_cnts.values()):,}",
            sub=f"across {len(outlier_cnts)} column(s)",
            color="amber",
        ))
...
# ─────────────────────────────────────────────────────────────────────────────
# Chart trace builders
# ─────────────────────────────────────────────────────────────────────────────

PALETTE = ["#3b82f6","#10b981","#f59e0b","#8b5cf6","#ef4444","#06b6d4","#ec4899","#84cc16"]


def _histogram_traces(s: pd.Series, col: str, bounds: Optional[tuple]) -> List[Dict]:
    traces = [{
        "type": "histogram", "x": s.dropna().tolist(), "name": col,
        "marker": {"color": "rgba(59,130,246,0.7)", "line": {"color": "#2563eb", "width": 1}},
        "autobinx": True,
    }]
    if bounds:
        lo, hi   = bounds
        outliers = s[(s < lo) | (s > hi)].dropna().tolist()
        if outliers:
            traces.append({
                "type": "scatter", "mode": "markers",
                "x": outliers, "y": [0] * len(outliers), "name": "Outliers",
                "marker": {"color": "#ef4444", "size": 10, "symbol": "circle-open",
                           "line": {"width": 2}},
            })
    return traces


def _box_traces(s: pd.Series, col: str) -> List[Dict]:
    return [{"type": "box", "y": s.dropna().tolist(), "name": col,
             "boxpoints": "outliers",
             "marker": {"color": "#8b5cf6", "outliercolor": "#ef4444", "size": 5},
             "line": {"color": "#7c3aed"}, "fillcolor": "rgba(139,92,246,0.15)"}]


def _bar_traces(vc: pd.Series, col: str) -> List[Dict]:
    top = vc.head(15)
    return [{"type": "bar", "x": top.index.astype(str).tolist(), "y": top.values.tolist(),
             "name": col,
             "marker": {"color": [PALETTE[i % len(PALETTE)] for i in range(len(top))],
                        "line": {"color": "white", "width": 1}}}]


def _pie_traces(vc: pd.Series) -> List[Dict]:
    top = vc.head(10)
    return [{"type": "pie", "labels": top.index.astype(str).tolist(),
...
def _line_traces(df: pd.DataFrame, date_col: str, num_cols: List[str]) -> List[Dict]:
    traces: List[Dict] = []
    try:
...
def _heatmap_traces(corr: pd.DataFrame) -> List[Dict]:
    cols = corr.columns.tolist()
    z    = [[round(corr.loc[r, c], 3) for c in cols] for r in cols]
    return [{"type": "heatmap", "x": cols, "y": cols, "z": z,
...
def _build_charts(df, col_meta, outlier_cnts, corr_df) -> List[ChartSpec]:
    charts: List[ChartSpec] = []
    chart_id = 0
    ...
    for col in num_cols[:6]:
        ...
        charts.append(ChartSpec(
            id=nxt(), title=f"Distribution — {col}", type="histogram", column=col,
            traces=_histogram_traces(s, col, bounds),
            layout={"xaxis": {"title": col}, "yaxis": {"title": "Frequency"}},
            anomaly_message=anom,
        ))
        charts.append(ChartSpec(
            id=nxt(), title=f"Box Plot — {col}", type="box", column=col,
            traces=_box_traces(s, col),
    ...
    for col in cat_cols[:5]:
        ...
        if vc.empty:
            continue
        charts.append(ChartSpec(
            id=nxt(), title=f"Frequency — {col}", type="bar", column=col,
            traces=_bar_traces(vc, col),
            layout={"xaxis": {"title": col, "tickangle": -35}, "yaxis": {"title": "Count"}},
        ))
        if vc.nunique() <= 12:
            charts.append(ChartSpec(
                id=nxt(), title=f"Breakdown — {col}", type="pie", column=col,
                traces=_pie_traces(vc),
            ))
    ...
    if date_cols and num_cols:
        ...
        if traces:
            charts.append(ChartSpec(
                id=nxt(), title=f"Trend Over Time — {date_cols[0]}", type="line",
                column=date_cols[0], traces=traces,
                layout={"xaxis": {"title": date_cols[0]}, "yaxis": {"title": "Value"},
                        "legend": {"orientation": "h"}},
            ))

    if corr_df is not None and len(corr_df) >= 2:
        charts.append(ChartSpec(
            id=nxt(), title="Correlation Heatmap", type="heatmap", column="correlation",
...
# ─────────────────────────────────────────────────────────────────────────────
# ✅ ENDPOINT — /api/dashboard/auto  (matches frontend calls)
#
# main.py registers:  app.include_router(dashboard_router)   ← no prefix
# decorator has:      /api/dashboard/auto                    ← full path
# Final URL:          /api/dashboard/auto                    ✓
# ─────────────────────────────────────────────────────────────────────────────
...
async def auto_dashboard(
    ...
    """Dynamic dashboard for ANY CSV/Excel file. No hardcoded columns."""
    ...
    numeric_summary: Dict[str, Dict[str, Any]] = {}
    if num_cols:
        numeric_summary = _jsonify(df[num_cols].describe().to_dict())
